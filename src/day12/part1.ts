...
export function part1(input: string): number {
            ...
            for (let dy = -1; dy <= 1; dy++) {
                for (let dx = -1; dx <= 1; dx++) {
                    ...
                    if (neighbourHeight <= currentHeight + 1) {
                        ...
                        g.setEdge(nodeName, neighbourName);
                    }
                }
            }
        }
    ...
    const paths = alg.dijkstra(g, start);
    const endNode = paths[end];
    if (!isDefined(endNode)) throw new ReferenceError('endNode is undefined');
